...
echo 'Test connexion MQTTS depuis web_app:'
nc -z mosquitto 8883 && echo 'OK' || echo 'FAIL'
...
echo 'Test connexion MQTTS depuis mariadb:'
nc -z mosquitto 8883 && echo 'OK' || echo 'FAIL'
...
docker run --rm --network marais_net alpine sh -c "
...
nc -zv mosquitto 8883 && echo 'MQTTS PORT OK' || echo 'MQTTS PORT FAIL'
...
docker logs --tail=5 web_app | grep -i error || echo "Aucune erreur trouvée"

echo "mariadb erreurs:"
docker logs --tail=5 mariadb | grep -i error || echo "Aucune erreur trouvée"

echo "mosquitto erreurs:"
docker logs --tail=5 mosquitto | grep -i error || echo "Aucune erreur trouvée"
